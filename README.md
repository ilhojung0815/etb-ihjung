...
1. festival 페이지에 서버단의 데이터들을 가져오던 중 다른 데이터들은 성공적으로 수신 되었으나, image는 정상적으로 페이지에 표기되지 않았는데 이에 대한 몇 가지 원인을 발견하였다.
        ...
        (2). image entity를 지정 후 이미지는 출력이 되었으나, 하나의 이미지만 중복해서 출력이 되었는데, 이는 index 없이 이미지를 출력하였기에 가장 먼저 출력된 이미지만 중복해서 나오게 되었다. 

        (3). 즉 다시 말하면, festivalArticle과 image 사이의 연결점이 하나도 없었기에 발생한 문제였다. 이를 확인하고 image에 index라는 연결점을 추가한 결과 해당 문제가 해결하였다.

  <br><br><br>
 
2. 관리자페이지(admin)에서 이미지를 서버로 업로드 하는 과정에서 오류가 발생, 확인한 결과 Front단과 Back단에서 이미지를 주고받을 때 Front에서는 하나의 파일만 전송하도록 되어있었으나 Back에서는 MultipartFile[] 같은 배열형태로 타입형태가 일치하지 않아 발생한 오류였다.<br>
...
💡 먼저 초기에는 수정, 삭제에 관리자 권한을 부여하는 구문을<br>
...
# 7. 마치며
<div>이번 프로젝트는 학부를 졸업한 뒤 처음으로 진행하는 웹개발 팀프로젝트였다. 졸업 후 2년이라는 긴 시간동안의 공백 후 처음으로 진행하는 프로젝트라 잘할 수 있을까에 대한 걱정도 많았지만 좋은 팀원들을 만나서 서로 많이 서툰 실력들이지만 해결하기 어려웠던 문제들을 다같이 해결해나가고 응원하면서 많은 것들을 배울 수 있었던 기회였다. 기술을 학습하는 것도 의미가 있었지만, 팀이라는 것에 대해 한번 더 생각하고 배울 수 있었던 점이 이번 프로젝트에서 가장 큰 성과였던 것 같다.</div>
